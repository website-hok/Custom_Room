...
if (localStorage.getItem("wzzdy_createtip") != "0.2") {
    mdui.alert({
        headline: "提示",
        description: "房主打开游戏训练营选择英雄后 进入加载页时打开网页 点击解除限制跳转游戏即可解除不满人无法开启的限制 其他人直接使用生成的链接进入房间即可",
        confirmText: "我知道了",
...
allbutton[1].onclick = function () {
    ...
    var span = document.createElement('span');
    // 设置<span>元素的文本内容  
    span.innerHTML = '<span slot="description">你可以通过编辑「生成规则」来编辑复制链接的生成规则</span>';
    mdui.dialog({
        headline: "提示",
        description: span,
        actions: [
            ...
            },
            {
                ...
                onClick: () => {
                    if (window.openurl) {
                        ...
                        getShortLink(window.location.origin + "/main/cr/Smoba.html?data=" + openurl)
                            .then(shortLink => {
                                ...
                                mdui.confirm({
                                    ...
                                    cancelText: "取消",
                                    onConfirm: () => {
                                        let url = replaceContent(myedit.value, window.location.origin + "/main/cr/data.html?" + murl, 0, openurl)
                                        showqr(url, function () {
                                            复制文本(url)
                                        })
                                    },
                            ...
                            .catch(error => {
                                ...
                                mdui.alert({
                                    headline: "提示",
                                    description: "出现错误 无法请求 请检查网络或重试几次",
                                    confirmText: "我知道了",
                                    onConfirm: () => console.log("confirmed"),
                                });
                                console.log(error)
...
allbutton[3].onclick = function () {
    ...
    mdui.confirm({
        headline: "提示",
        description: "是否更新英雄列表 点击继续获取 成功后会自动刷新网页 当之后英雄列表变化可以手动点击更新英雄列表更新列表哦",
        confirmText: "继续",
        cancelText: "取消",
        onConfirm: () => {
